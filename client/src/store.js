...
export default new Vuex.Store({
  ...
  actions: {
    ...
    getAllQuestion ({ commit }, payload) {
      ...
      axios({
        method: 'post',
        url: 'http://35.240.205.140/question/'
      }).then(dataQuestion => {
    ...
    getAnswer ({ commit }, payload) {
      axios({
        method: 'get',
        url: `http://35.240.205.140/answer/${payload}`
      }).then(response => {
    ...
    addAnswer ({ commit }, payload) {
      ...
      axios({
        method: 'post',
        url: `http://35.240.205.140/question/detail/answer`,
        data: {
    ...
    upVoteQuestion ({ commit }, payload) {
      if (String(this.state.userData) == String(this.state.detail.UserId._id)) {
        swal({
          text: "Tidak bisa Upvote pertanyaan sendiri !",
          icon: "error",
        });
      } else {
        const token = localStorage.getItem('authorization').split('*#$_.')[0]
        axios({
          method: 'post',
          url: `http://35.240.205.140/question/detail/upvote/${payload}`,
          data: {
            token: token
          }
        }).then(response => {
    ...
    downVoteQuestion ({ commit }, payload) {
      if (String(this.state.userData) == String(this.state.detail.UserId._id)) {
        swal({
          text: "Tidak bisa Downvote pertanyaan sendiri !",
          icon: "error",
        });
      } else {
        const token = localStorage.getItem('authorization').split('*#$_.')[0]
        axios({
          method: 'post',
          url: `http://35.240.205.140/question/detail/downvote/${payload}`,
          data: {
            token: token
          }
        }).then(response => {
    ...
    upVoteAnswer ({ commit }, payload) {
      ...
      axios({
        method: 'post',
        url: `http://35.240.205.140/question/detail/upvoteanswer/${String(payload)}`,
        data: {
          questionId: payload,
          token: token
        }
    ...
    downVoteAnswer ({ commit }, payload) {
      ...
      axios({
        method: 'post',
        url: `http://35.240.205.140/question/detail/downvoteanswer/${payload.answerId}`,
        data: {
          questionId: payload,
          token: token
        }
